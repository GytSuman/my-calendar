...
function Time(props) {
	...
	const { state, dispatch, openAllEventsWeek, setOpenAllEventsWeek } =
		useCalendar();
				...
				{state.allEvents &&
					state.allEvents.map((event) => (
						<>
							{console.log(event)}
							{event.dayOfTheYear ===
								dayjs(day.dateStamp).format("DD MM YYYY") &&
	...
	React.useEffect(() => {
		if (
			dayjs(day.dateStamp).hour(time).format("DD/MM/YYYY HH:mm") !==
			openAllEventsWeek
		)
			setOpen(false);
	}, [openAllEventsWeek]);
				...
				onClick={() => {
					...
					dispatch({
						...
						payload: {
							...
							weekDateStamp: dayjs(day.dateStamp).hour(time),
							dayOfTheYear: dayjs(day.dateStamp).format("DD MM YYYY"),
							time,
							id: day.id,
						},
					});
							...
							<div>
								{state.allEvents &&
									currEvents(state.allEvents)
										.slice(2, currEvents(state.allEvents).length)
										.map((event) => <Event type="voice" event={event} />)}
							</div>
						</div>
					</span>
				)}
