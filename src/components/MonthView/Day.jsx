		...
		>
			...
			<div>
				...
				>
					...
					{
						nEvents > 3 && (
							<div className="themeblue-bg border-2 flex-center flex-center2 font-12 white-color" style={{ width: '38px', height: '21px' }}>
								+{nEvents - 3}
							</div>
						)
					}
				</div>
				<div style={{ height: '100px', marginTop: '20px', overflowY: 'hidden' }}>
						...
						state.allEvents.map((eventObj) => (
							<>
								<div key={eventObj.id}>
									...
									{dayjs(eventObj.dateStamp).format("DD MM YYYY") === day.format("DD MM YYYY") &&
										(
											<>
												<Event type="voice" eventObj={eventObj} key={eventObj.id} />
											</>
